...
from app.auth import get_current_user # Dùng chung dependency với web
...
# --- Admin Endpoints ---
# Loại bỏ dependency cũ, dùng chung dependency mới
@router.post("/keys/add", summary="Thêm một key mới")
async def add_new_key(request: KeyRequest, user_logged_in: bool = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.post("/keys/delete", summary="Xóa một key")
async def delete_existing_key(request: KeyRequest, user_logged_in: bool = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user_logged_in:
        raise HTTPException(status_code=401, detail="Unauthorized")
...
@router.get("/keys/status/{key_value}", summary="Kiểm tra trạng thái một key")
async def check_key_status(key_value: str, user_logged_in: bool = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user_logged_in:
        raise HTTPException(status_code=401, detail="Unauthorized")
