...
async def handle_add_key(key_value: str = Form(...), user_logged_in: bool = Depends(get_current_user)):
    """Xử lý việc thêm key mới."""
    if not user_logged_in:
        return RedirectResponse(url="/admin/login")
    
    if key_value:
        key_service.add_key(key_value)
    
    return RedirectResponse(url="/admin/dashboard", status_code=status.HTTP_302_FOUND)

@router.post("/admin/delete-key", response_class=RedirectResponse)
async def handle_delete_key(key_value: str = Form(...), user_logged_in: bool = Depends(get_current_user)):
    """Xử lý việc xóa key."""
    if not user_logged_in:
        return RedirectResponse(url="/admin/login")
    
    if key_value:
        key_service.delete_key(key_value)
